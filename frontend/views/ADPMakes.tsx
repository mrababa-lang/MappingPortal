...
export const ADPMakesView: React.FC = () => {
  ...
  // Pagination Logic
  const totalPages = Math.ceil(uniqueMakes.length / ITEMS_PER_PAGE);
  const paginatedMakes = uniqueMakes.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
              ...
              {uniqueMakes.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                    No unique makes found in ADP Master data.
                  </td>
                </tr>
              )}
            </tbody>
